...
class AdvancedProductDesigner
{
    ...
    private function clean_svg_for_cutting($svg_content, $order_id = 0)
    {
        ...
        if (!$loaded) {
            ...
            error_log('APD SVG Processing Error - Order #' . $order_id . ': ' . implode('; ', $error_messages));
                error_log('APD SVG Content (first 500 chars): ' . substr($svg_content, 0, 500));
                return new WP_Error('invalid_xml', 'Invalid SVG XML structure. Check error log for details.');
            }
        }
        ...
        // 2. Handle material stroke patterns - preserve stroke attributes but remove pattern references
        // First, fix elements that use patterns for strokes (material outlines)
        $strokeRefs = $xpath->query('//*[@stroke and contains(@stroke, "url(#")]');
        $stroke_count = 0;
        foreach ($strokeRefs as $elem) {
            $stroke = $elem->getAttribute('stroke');
            if (preg_match('/url\(#.*pattern.*\)/i', $stroke)) {
                // Keep the stroke but change to solid black (for cut lines)
                // Preserve stroke-width and other stroke attributes
                $stroke_width = $elem->getAttribute('stroke-width');
                $elem->setAttribute('stroke', '#000000');
                $stroke_count++;
                // Log what we found for debugging
                error_log(sprintf(
                    'APD Cut-Ready SVG - Order #%d: Converted pattern stroke on <%s> element with stroke-width=%s',
                    $order_id,
                    $elem->nodeName,
                    $stroke_width ?: 'none'
                ));
                // These elements should keep their stroke attributes for CorelDRAW
                // The stroke represents the material outline which is important for cutting
            }
        }
        error_log(sprintf('APD Cut-Ready SVG - Order #%d: Converted %d pattern strokes to solid black', $order_id, $stroke_count));
        ...
        foreach ($nestedSvgs as $nested) {
            ...
            if ($nested->hasAttribute('transform')) {
                $g->setAttribute('transform', $nested->getAttribute('transform'));
            }
        ...
        // 8. Clean up and return with UTF-16 encoding
    ...
    private function fix_common_xml_issues($svg_content)
    {
        ...
        // Remove HTML-style inline style attributes with invalid XML content
        ...
        // Fix shape-rendering and text-rendering attributes with hyphens (not valid in strict XML)
        $svg_content = str_replace('shape-rendering', 'shapeRendering', $svg_content);
        $svg_content = str_replace('text-rendering', 'textRendering', $svg_content);
