    ...
    const FSC = {
        ...
        ensureMaterialGroup: function(){
            ...
            var renderMaterialGroup = function(materialsMap){
                ...
                $('.fsc-material-outline-option').off('click.apdMatGrid').on('click.apdMatGrid', function(){
                    ...
                    FSC.updatePreview();
                    // Apply material outline to text
                    if (FSC.scheduleApplyTextOutlineAll) {
                        FSC.scheduleApplyTextOutlineAll();
                    }
                    // Apply material outline to logo
                    if (FSC.applyLogoMaterialOutline) {
                        setTimeout(function(){
                            FSC.applyLogoMaterialOutline();
                        }, 50);
                    }
        ...
        bindInputEvents: function() {
            ...
            $(document).on('click', '.fsc-material-outline-option', function() {
                ...
                $(this).addClass('selected');
                FSC.currentMaterial = $(this).data('material');
                FSC.currentMaterialUrl = $(this).data('material-url') || $(this).data('url');
                FSC.scheduleUpdatePreview();
                // Apply material outline to text
                if (FSC.scheduleApplyTextOutlineAll) {
                    FSC.scheduleApplyTextOutlineAll();
        ...
        mergeSavedTemplateDataIntoCache: function(productData) {
            try {
                ...
                // Restore global line height (one for all custom text)
                if (saved.lineHeight !== undefined) {
                    FSC._cachedTemplateData.lineHeight = saved.lineHeight;
                }
            ...
            (elements || []).forEach(function(el) {
                ...
                if (el.type === 'logo' && productData.logo_content) {
                    ...
                    $(document).off('click.apdMatSel').on('click.apdMatSel', '.fsc-material-outline-option', function(){
                        ...
                        $(this).addClass('selected');
                        applyLogoPattern();
                        // Also apply to all logo outlines using unified function
                        if (FSC.applyLogoMaterialOutline) {
                            setTimeout(function(){
                                FSC.applyLogoMaterialOutline();
                            }, 50);
        ...
        renderControls: function(templateData) {
            ...
            // Unified material outline handler for both logo and text
        ...
        // Apply material outline to logo outline layer
