    ...
    private function clean_svg_for_cutting($svg_content, $order_id = 0)
    {
        ...
        foreach ($styles as $style) {
            ...
            if (stripos($styleContent, '@font-face') !== false && 
                ...
                // IMPORTANT: Only match @font-face blocks, preserve all other CSS rules
                ...
                // Pattern 4: If base64 is extremely long and spans many lines, [^}] won't match newlines
                // Use DOTALL mode with .*? to match everything including newlines until }
                if (stripos($cleanedContent, '@font-face') !== false && 
                    ...
                    // Use DOTALL mode (s flag) - . matches newlines, *? is non-greedy
                    $cleanedContent = preg_replace(
                        '/@font-face\s*\{.*?base64.*?\}/is',
                        '',
                        $cleanedContent
        ...
        */
        ...
        // CRITICAL: After all regex cleanup, reload DOM and restore ALL inline styles
        // This is a final safety check to ensure all inline styles from Original SVG are preserved
        // Reload the cleaned SVG back into DOM to restore styles properly
        ...
        if ($finalLoaded && $finalDom->documentElement) {
            ...
            $finalElements = $finalXpath->query('//*');
